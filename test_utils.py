...
def split_text(raw_text: str, chunk_size: int = 1000, chunk_overlap: int = 100) -> list:
...
def initialize_qa_chain(retriever, api_key: str, model: str = "gemini-1.5-flash", temperature: float = 0.7):
    ...
    retriever_prompt = ChatPromptTemplate.from_messages([
        MessagesPlaceholder(variable_name="chat_history"),
        ("user", "{input}"),
        ("user", "Generate a concise search query based on the conversation to retrieve relevant HRIS document content.")
    ])
